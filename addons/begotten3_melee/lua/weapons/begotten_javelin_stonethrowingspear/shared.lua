...
SWEP.PrintName = "Stone Throwing Spear"
...
SWEP.AttackTable = "StoneThrowingSpearAttackTable"

SWEP.Primary.Round = ("begotten_javelin_stonethrowingspear_thrown");
...
function SWEP:HandlePrimaryAttack()
	...
	timer.Create("javelin_timer_"..self.Owner:EntIndex(), 0.5, 1, function()
		if IsValid(self) and IsValid(self.Owner) and self.Owner:GetActiveWeapon() and self.Owner:GetActiveWeapon().PrintName == "Stone Throwing Spear" then
			self:FireJavelin();
...
function SWEP:FireJavelin()
	...
	if SERVER then
		...
		javelin:SetModel("models/weapons/darky_m/rust/w_stone_spear.mdl");
	...
	if SERVER and owner:IsPlayer() then
		...
		if owner.opponent then
			...
			if owner.StripWeapon then
				owner:StripWeapon("begotten_javelin_stonethrowingspear");
			end
		else
...
SWEP.VElements = {
	["v_stonethrowingspear"] = { type = "Model", model = "models/weapons/darky_m/rust/w_stone_spear.mdl", bone = "v_weapon.Knife_Handle", rel = "", pos = Vector(3.5, -1.481, -7.407), angle = Angle(105, 0, 82.986), size = Vector(1, 1, 1), material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["w_stonethrowingspear"] = { type = "Model", model = "models/weapons/darky_m/rust/w_stone_spear.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(4.9, -0.5, -12.346), angle = Angle(97.013, 40.909, 54.935), size = Vector(1, 1, 1), material = "", skin = 0, bodygroup = {} }
}

SWEP.WElementsAlternate = {
	["w_stonethrowingspear"] = { type = "Model", model = "models/weapons/darky_m/rust/w_stone_spear.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(2.469, 1, 6.420), angle = Angle(-97, -110, 70), size = Vector(1, 1, 1), material = "", skin = 0, bodygroup = {} }
}