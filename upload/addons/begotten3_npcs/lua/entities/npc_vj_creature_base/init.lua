		...
		if self:GetEnemy() != nil then
			...
			if self.HasMeleeAttack == true then
				...
				if self:CanDoCertainAttack("MeleeAttack") == true then
					...
					if isbeingcontrolled == false && self.NearestPointToEnemyDistance < self.MeleeAttackDistance && self:GetEnemy():Visible(self) then ispropattack = false isnormalattack = true end
...
function ENT:CreateDeathCorpse(dmginfo,hitgroup)
	...
	if self.HasDeathRagdoll == true then
		...
		if GetConVarNumber("vj_npc_corpsefade") == 1 or GetConVarNumber("vj_npc_corpsefade") == 0 then self.Corpse:Fire(self.Corpse.FadeCorpseType,"",250) end
...
--<>-- Convars that run on Initialize --<>--
	...
	if GetConVarNumber("vj_npc_nopropattack") == 0 or ("vj_npc_nopropattack") == 1 then self.AttackProps = false end
