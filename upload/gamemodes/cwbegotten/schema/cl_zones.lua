...
local HDR_LEVEL = GetConVar("mat_hdr_level"):GetInt();
		...
		if (self:FogEnabled()) then
			if (zoneTable.fogColors and self.currentFogColors) then
				if (!fogColorsUpdated) then
					local zoneFogColors = zoneTable.fogColors;
					local interval = zoneTable.colorInterval or 64;
					local bNoFade = false;
					
					if zoneTable.hasNight and cwDayNight and cwDayNight.nightWeight then 
	...
	if (self.currentZoneTable) then
		...
		if zoneTable.hasNight and cwDayNight and cwDayNight.nightWeight and !Clockwork.Client.dueling and !Clockwork.kernel:IsChoosingCharacter() then
			if skyFix and zoneTable.skyFixNight then
				local skyFixNight = zoneTable.skyFixNight;
...
if (game.GetMap() == "rp_temple") then
-- Called just before the skybox is drawn.
function zones:PreDraw2DSkyBox()
	...
	if (self:Enabled() and self:FogEnabled() and self.currentFogColors) then
		...
		if zoneTable then
			if (zoneTable.default) --[[or zoneTable.name == "tower"]] then
				if Clockwork.Client.currentCycle == "day" or Clockwork.Client.currentCycle == "night" then
					if !Clockwork.Client.dueling and !Clockwork.kernel:IsChoosingCharacter() then
						return;
					end
				end
...
Senior Imperial Minister Konstadinos Mathghamhain 'Steed' Nerva II, Master of Cannons has spawned 'models/props_c17/truss03b.mdl'.

Senior Imperial Minister Konstadinos Mathghamhain 'Steed' Nerva II, Master of Cannons has spawned 'models/props_c17/truss03b.mdl'.

[OOC] gabs: I wish the snow didnt look slightly green

] cwlua "print(Clockwork.Client:GetPos())"
