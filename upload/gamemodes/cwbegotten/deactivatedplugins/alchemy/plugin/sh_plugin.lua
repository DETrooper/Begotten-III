		...
		if transleft > 0 then
			if amounttopour > 0 then
				...
				if danger.unstable > 0 then
					Clockwork.chatBox:AddInTargetRadius(player, "me", "very carefully starts transferring the contents of their "..string.lower(name).." to a "..string.lower(targetname).."...", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
				else
					Clockwork.chatBox:AddInTargetRadius(player, "me", "starts transferring the contents of their "..string.lower(name).." to a "..string.lower(targetname).."...", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
				end
				...
				Clockwork.player:SetAction(player, "transferring_contents_container", transfer.total/200, 5, function()
					if cwAlchemy:DisturbContainer(player, containerto, buffer.total/1000) then
						...
						Clockwork.chatBox:AddInTargetRadius(player, "me", "finishes transferring contents to their "..string.lower(targetname)..".", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
...
function cwAlchemy:Dissolve(player,uniqueid1,itemid1,uniqueid2,itemid2)
	...
	if (dissolve and containerto) then
		...
		if transleft > 0 then
			...
			local danger = cwAlchemy:CalcExplosiveStats(buffer.output, 0)
			if danger.unstable > 0 then
				Clockwork.chatBox:AddInTargetRadius(player, "me", "very carefully puts a "..string.lower(name).." in their "..string.lower(targetname).." and watches it dissolve...", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
			else
				Clockwork.chatBox:AddInTargetRadius(player, "me", "puts a "..string.lower(name).." in their "..string.lower(targetname).." and watches it dissolve...", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
			end
			Clockwork.player:SetAction(player, "transferring_contents_container", transfer.total/200, 5, function()
				-- add the amount removed from the first container to the second
				...
				Clockwork.chatBox:AddInTargetRadius(player, "me", "finishes dissolving a "..string.lower(name)..".", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
				--the container has been handled, trigger DisturbContainer with the shock argument being the number of liters transferred
