--[[
	Credit goes to Panto for making the original fix for ai.GetTaskID.
	http://www.facepunch.com/member.php?u=466934
]]--

cwZombies.aiTask = library.New("AiTask", cwZombies);
cwZombies.aiTask.stored = {
	["TASK_INVALID"] = 0,
...
function cwZombies.aiTask:GetTaskID(taskName)
	if (!self.stored[taskName]) then
		return 0;
	end;

	return self.stored[taskName];
end;

function ai.GetTaskID(taskName)
	return cwZombies.aiTask:GetTaskID(taskName);
end;