...
function PLUGIN:Move(player, cmovedata)
        local isJumpNew = bit.band(cmovedata:GetButtons(), IN_JUMP) != 0 and bit.band(cmovedata:GetOldButtons(), IN_JUMP) == 0

    	if (player:Alive() and player:GetMoveType() != MOVETYPE_NOCLIP) and player:IsOnGround() and isJumpNew then
