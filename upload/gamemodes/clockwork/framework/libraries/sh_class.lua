...
if (Clockwork.class) then return end

library.New("class", Clockwork)
local stored = {}
local buffer = {}

function Clockwork.class:GetStored()
	return stored
end

function Clockwork.class:GetBuffer()
	return buffer
end
...
function Clockwork.class:GetLimit(name)
	...
	if (class) then
		if (class.limit != 128) then
			return math.ceil(class.limit / (128 / _player.GetCount()))
		else
