...
function Clockwork.entity:IsInBox(entity, minimum, maximum)
	...
	if (entity:IsPlayer() or entity:IsNPC() or entitiy:IsNextBot()) then
...
function Clockwork.entity:IsPlayerRagdoll(entity)
	...
	if (IsValid(player)) then
		...
		if IsValid(ragdollEntity) and (player:GetRagdollEntity() == entity) then
			return true
		end
	end
end
...
function Clockwork.entity:IsInteractable(entity)
	...
	if (entity:IsNPC() or entity:IsNextBot() or entity:IsPlayer() or string.find(class, "prop_dynamic")) then
		return false
	end
...
if (SERVER) then
	...
	function Clockwork.entity:BelongsToAnotherCharacter(player, entity)
		...
		if (uniqueID and key) then
			if (player.UniqueID and uniqueID == player:UniqueID() and key != player:GetCharacterKey()) then
				return true
			end
		end
