...
local IsDoorLocked = Clockwork.entity.IsDoorLocked
local GetDoorState = Clockwork.entity.GetDoorState
	...
	for k, v in pairs(Clockwork.entity:GetDoorEntities()) do
		if (v:IsValid()) then
			doorTable[#doorTable + 1] = {
				position = v:GetPos(),
				bLocked = IsDoorLocked(Clockwork.entity, v),
				state = GetDoorState(Clockwork.entity, v)
			}
		end
	end
