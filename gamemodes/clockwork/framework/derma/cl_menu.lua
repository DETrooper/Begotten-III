			...
			if (dropped) then
				...
				if panel then
					...
					if parent and parent.itemData and parent.itemTable then
						if Clockwork.Client:GetItemEquipped(parent.itemTable) then
							netstream.Start("UnequipItem", {parent.itemTable("uniqueID"), parent.itemTable("itemID"), "drop"});
						else
							Clockwork.inventory:InventoryAction("drop", parent.itemTable.uniqueID, parent.itemTable.itemID);
						end
					end
				end
			end
	...
	if (!hook.Run("PreMainMenuThink", self)) then
		...
		if self.statusInfo then
			if !Clockwork.ActiveDermaToolTips or table.IsEmpty(Clockwork.ActiveDermaToolTips) then
				self.statusInfo:SetMouseInputEnabled(true)
				self.statusInfo:SetKeyboardInputEnabled(true)
				--self.statusInfo:MakePopup();
			end
		end
	end;
