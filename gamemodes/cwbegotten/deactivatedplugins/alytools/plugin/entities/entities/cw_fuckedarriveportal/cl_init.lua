...
function ENT:Think()
	...
	self.bhSize = self:GetNWFloat( "wormholesize" ) or 8
...
function ENT:Draw( )
	...
	local core = self:GetPos() + Vector( 0, 0, 41*self:GetModelScale() )
	...
	for i=0, math.random(1,4) do 
		...
		local particle = emitter:Add( "effects/spark", core  )
		if particle then
			particle:SetAngles( randang * -1 )
			particle:SetVelocity( offtemp )
			particle:SetColor( 255, 70, 70, 255 )
