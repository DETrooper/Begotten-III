local ITEM = Clockwork.item:New(nil, true);
	...
	function ITEM:OnCustomFunction(player, name)
		...
		if (name == "Chug") then
			if data.volume > 0 then
				Clockwork.chatBox:AddInTargetRadius(player, "me", "lifts their "..string.lower(self.name).." to their lips and starts chugging.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
		...
		elseif (name == "Dump Contents") then
			if data.volume > 0 then
				Clockwork.chatBox:AddInTargetRadius(player, "me", "tips their "..string.lower(self.name).." and pours out the contents onto the ground.", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
		...
		elseif (name == "Pour On Self") then
			if data.volume > 0 then
				Clockwork.chatBox:AddInTargetRadius(player, "me", "raises their "..string.lower(self.name).." over their head and dumps the contents all over themselves!", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
	...
	function ITEM:OnUse(player, itemEntity)
		...
		if data.volume > 0 then
			...
			cwAlchemy:DrinkChem(player,drink)
			self:SetData("ChemContents",drink.remaining)
			Clockwork.chatBox:AddInTargetRadius(player, "me", "takes a drink from their "..string.lower(self.name)..".", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
			Clockwork.player:Notify(player, "You drink "..cwAlchemy:ConvertUnits(drink.total)..".");
		else
			Clockwork.player:Notify(player, "This container is completely empty.");
		end
...
local ITEM = Clockwork.item:New(nil, true);
	...
	function ITEM:OnUse(player, itemEntity)
		...
		Clockwork.chatBox:AddInTargetRadius(player, "me", "swallows a "..string.lower(self.name)..".", player:GetPos(), Clockwork.config:Get("talk_radius"):Get() * 2);
	end;
