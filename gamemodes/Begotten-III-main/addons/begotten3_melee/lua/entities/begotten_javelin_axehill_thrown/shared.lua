ENT.Type 			= "anim"
ENT.PrintName		= "Hillkeeper Throwing Axe"
ENT.Author			= ""
ENT.Contact			= ""
ENT.Purpose			= ""
ENT.Instructions		= ""
...
if SERVER then
	...
	function ENT:Initialize()
		...
		if IsValid(self.Owner) and self.Owner:GetActiveWeapon().isJavelin then
			...
			self.itemTable = item.CreateInstance("begotten_javelin_axehill");
		end
		...
		if !self.collided then
			...
			if Ent:IsWorld() and self.InFlight then
				if data.Speed > 500 then
					...
					self:SetPos(data.HitPos - (data.HitNormal * 15))
			...
			elseif Ent.Health and IsValid(self.Owner) then
				...
				if self.itemTable then
					...
					if condition and condition < 100 then
						...
						minDamage = math.Round(minDamage * Lerp(scalar, 0.4, 1));
						maxDamage = math.Round(maxDamage * Lerp(scalar, 0.4, 1));
					end
				end
